...
type Protocol struct {
	...
	//JL add PubSubList
	pubSubList map[string]Subscriber
}
...
// JL GetPubSubList
func (p *Protocol) GetPubSubList() map[string]Subscriber {
	return p.pubSubList
}
...
func (p *Protocol) Handle(ctx GRING.HandlerContext) error {
	...
	case P2pMessage:
		...
		if msg.Opcode == OP_REQUEST_REGROUP {
			...
			if err != nil {
				fmt.Printf("OP_REQUEST_REGROUP: Error on decode process: %v\n", err)
			}
			fmt.Printf("recv OP_REQUEST_REGROUP, CurrentRound : %d\n", regroup_data.NextRound)
			...
			if p.events.OnSelectPeers != nil {
				randomPeers := p.events.OnSelectPeers(regroup_data.Peers, regroup_data.NextRound)
				data, _ := json.Marshal(randomPeers)
				msg.Contents = data
			}
			...
			//I2 Start Train
			if p.events.OnStartTrainOneRound != nil {
				p.events.OnStartTrainOneRound(regroup_data.ModelData)
			}
		...
		if msg.Opcode == OP_REQUEST_GROUP {
			...
			//JL set pubSubList
			var pubSubList map[string]Subscriber
			err := json.Unmarshal(msg.Contents, &pubSubList)
			if err != nil {
				fmt.Printf("OP_REQUEST_GROUP: Error on decode process: %v\n", err)
			}
			p.pubSubList = pubSubList
...
func (p *Protocol) OnRequestGroupInitiator(msg P2pMessage) {
	...
	if len(sortedArr)-1 <= p.group_size-1 {
		...
		//JL
		fmt.Printf("OnRequestGroupInitiator Start, Initiator %s, #AllNodes(I-W) %d\n", p.node.ID().Address, len(sortedArr))
	...
	*/
	...
	//JL
	fmt.Printf("OnRequestGroupInitiator Start, Initiator %s #AllNodes(I-S-W) %d\n", p.node.ID().Address, len(sortedArr))
